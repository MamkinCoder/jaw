<?php
try {
    // Establish the PDO connection
    $pdo = new PDO("pgsql:host=your_database_host;port=your_database_port;dbname=your_database_name", "your_database_user", "your_database_password");
    
    // Handle the POST data from the Fetch API
    $data = json_decode(file_get_contents('php://input'), true);
    $name = $data['name'];
    $age = $data['age'];
    $gender = $data['gender'];
    $email = $data['email'];
    $q1 = $data['q1'];
    $q2 = $data['q2'];
    $q3 = $data['q3'];
    $q4 = $data['q4'];
    $q5 = $data['q5'];
    $q6 = $data['q6'];
    $q7 = $data['q7'];
    $q8 = $data['q8'];
    $q9 = $data['q9'];
    $q10 = $data['q10'];
    $q11 = $data['q11'];
    $q12 = $data['q12'];
    $q13 = $data['q13'];
    
    // Prepare the INSERT statement
    $stmt = $pdo->prepare('INSERT INTO person (name, age, gender, email, q1, q2, q3, q4, q5, q6, q7, q8, q9, q10, q11, q12, q13)
                           VALUES (:name, :age, :gender, :email, :q1, :q2, :q3, :q4, :q5, :q6, :q7, :q8, :q9, :q10, :q11, :q12, :q13)');

    // Bind parameters and execute the INSERT query
    $stmt->bindParam(':name', $name, PDO::PARAM_STR);
    $stmt->bindParam(':age', $age, PDO::PARAM_INT);
    $stmt->bindParam(':gender', $gender, PDO::PARAM_BOOL);
    $stmt->bindParam(':email', $email, PDO::PARAM_STR);
    $stmt->bindParam(':q1', $q1, PDO::PARAM_INT | PDO::PARAM_ARRAY);
    $stmt->bindParam(':q2', $q2, PDO::PARAM_INT);
    $stmt->bindParam(':q3', $q3, PDO::PARAM_INT | PDO::PARAM_ARRAY);
    $stmt->bindParam(':q4', $q4, PDO::PARAM_INT | PDO::PARAM_ARRAY);
    $stmt->bindParam(':q5', $q5, PDO::PARAM_INT);
    $stmt->bindParam(':q6', $q6, PDO::PARAM_INT);
    $stmt->bindParam(':q7', $q7, PDO::PARAM_INT);
    $stmt->bindParam(':q8', $q8, PDO::PARAM_INT);
    $stmt->bindParam(':q9', $q9, PDO::PARAM_INT | PDO::PARAM_ARRAY);
    $stmt->bindParam(':q10', $q10, PDO::PARAM_INT | PDO::PARAM_ARRAY);
    $stmt->bindParam(':q11', $q11, PDO::PARAM_STR);
    $stmt->bindParam(':q12', $q12, PDO::PARAM_INT | PDO::PARAM_ARRAY);
    $stmt->bindParam(':q13', $q13, PDO::PARAM_STR);

    $stmt->execute();

    echo "Data inserted successfully!";
} catch (PDOException $e) {
    die("Error: " . $e->getMessage());
}
?>
<!-- make it send error codes -->

<?php
try {
    // ... Same as before ...

    // Prepare the INSERT statement
    $stmt = $pdo->prepare('INSERT INTO person (name, age, gender, email, q1, q2, q3, q4, q5, q6, q7, q8, q9, q10, q11, q12, q13)
                           VALUES (:name, :age, :gender, :email, :q1, :q2, :q3, :q4, :q5, :q6, :q7, :q8, :q9, :q10, :q11, :q12, :q13)');

    // Bind parameters and execute the INSERT query
    // ... Same as before ...

    // Return success response
    http_response_code(200);
    echo json_encode(['status' => 'success', 'message' => 'Data inserted successfully!']);
} catch (PDOException $e) {
    // Return error response
    http_response_code(500);
    echo json_encode(['status' => 'error', 'message' => 'Error: ' . $e->getMessage()]);
}
?>
